...
# Testing script for Texy modified from MicroC's testall scripts
...
Usage() {
    echo "Usage: testall.sh [options] [.texy files]"
...
CheckFail() {
    ...
    RunFail "$TEXY" "<" $1 "2>" "${basename}.err" ">>" $globallog &&
...
then
    ...
    files="tests/test-*.texy tests/fail-*.texy"
fi
